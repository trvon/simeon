...
|--------|---------:|------------------:|--:|------------:|--:|
| scifact | 0.6188 | 0.6053 (`w=0.2`) | -0.0135 | 0.6170 (`w=0.2`) | -0.0018 |
...
|--------|---------:|--------------------:|--:|
| scifact | 0.6188 | 0.6188 (`k=3/8`, `lambda=0.2/0.5`) | +0.0000 |
...
The graph follow-up swaps the direct phrase bonus for a pool-restricted personalized-PageRank walk over query, phrase, and BM25 top-K document nodes, with an optional doc-doc overlap graph inside the pool.

| Corpus | Baseline | Best graph row | Δ |
|--------|---------:|---------------:|--:|
| scifact | 0.6188 | 0.6125 (`phrase_ppr k300 d0.85`) | -0.0063 |
...
The next follow-up keeps the document-anchored idea but swaps graph diffusion for **fragment covers**: top TextRank fragments per BM25-pooled doc are converted into sparse high-IDF signatures, greedily clustered by weighted overlap, then rescored by query-cover plus fragment-containment mass.

| Corpus | Baseline | Best cluster row | Δ |
|--------|---------:|-----------------:|--:|
...
The next follow-up keeps the fragment-anchored shape but replaces lexical overlap with **training-free PMI fragment vectors**. Top TextRank fragments per BM25-pooled doc are encoded with in-corpus PMI embeddings, connected by cosine similarity inside the pool, and rescored through a query-seeded fragment diffusion.

| Corpus | Baseline | Best fragment row | Δ |
|--------|---------:|------------------:|--:|
...
The hybrid follow-up keeps the PMI fragment graph backbone but adds two stronger-state tweaks: more fragments per document and a lexical bridge bonus on query-to-fragment plus fragment-to-fragment edges.

| Corpus | Baseline | Best hybrid row | Δ |
|--------|---------:|----------------:|--:|
...
The geometric follow-up keeps the PMI fragment backbone but replaces fixed-threshold graph edges with a **query-centered soft neighborhood**: locally whitened fragment vectors, softmax query attention, and kNN propagation in the whitened fragment space.

| Corpus | Baseline | Best geometric row | Δ |
|--------|---------:|-------------------:|--:|
...
The next follow-up tried to fix the corpus-sensitivity directly: keep the PMI fragment geometry, but adapt the BM25/geometry blend, neighborhood sharpness, kNN width, and propagation depth per query using cheap lexical regime signals (`avg_idf` and BM25 pool score decay).

| Corpus | Baseline | Best adaptive row | Δ |
|--------|---------:|------------------:|--:|
| scifact | 0.6188 | 0.6078 (`k100 t4 a0.70_0.98 s4_10 k4_16 p1_3`) | -0.0110 |
...
The next pass separated **better fragment state** from **better geometry control**. Three rows were added: a richer multi-scale fragment representation, a geometry-native gating row driven by query-to-fragment confidence signals, and a combined row.

| Corpus | Baseline | Best new ablation row | Δ |
|--------|---------:|----------------------:|--:|
| scifact | 0.6188 | 0.6128 (`rich_gsig`) | -0.0060 |
...
The next follow-up kept the richer multi-scale fragment state, but added overlap-based coverage controls so redundant sentence / anchor fragments were suppressed before the geometry step.

| Corpus | Baseline | Best coverage-controlled row | Δ |
|--------|---------:|-----------------------------:|--:|
| scifact | 0.6188 | 0.6161 (`richcov`) | -0.0027 |
...
The next follow-up made the coverage controls stricter by capping the retained state to at most 4 sentence fragments plus 1 novel anchor.

| Corpus | Baseline | Best budgeted row | Δ |
|--------|---------:|------------------:|--:|
| scifact | 0.6188 | 0.6113 (`richbud`) | -0.0075 |
...
The next follow-up replaced the hard fragment cap with a soft novelty-aware
selector over the same rich fragment candidates, using an MMR-style
`salience - lambda * redundancy` score and the existing `0.60 / 0.80`
sentence/anchor overlap caps as penalties rather than fixed count limits.

| Corpus | Baseline | Best MMR row | Δ |
|--------|---------:|-------------:|--:|
| scifact | 0.6188 | 0.6141 (`richmmr l0.50`) | -0.0047 |
